...
*   loadVoidInterceptorModel() / buildDetailedShipMesh() - Loads the solid binary glTF model (`fbx/void_interceptor.glb`, 450k faces, 9.2MB, 100% watertight boundary-preserved geometry) with original model orientation (`model.rotation.set(-Math.PI / 2, 0, -Math.PI / 2)`), twin engine exhaust glow assemblies (cavities, thruster discs, core discs, halos).
