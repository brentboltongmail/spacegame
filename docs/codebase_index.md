...
*   loadVoidInterceptorModel() / buildDetailedShipMesh() - Loads the master textured binary glTF model (`fbx/void_interceptor.glb`, full embedded 4K/2K diffuse, metallic, roughness maps) with original model orientation (`model.rotation.set(-Math.PI / 2, 0, -Math.PI / 2)`), twin engine exhaust glow assemblies (cavities, thruster discs, core discs, halos).
...
*   Carrier Fighter Deployment Loop (`launchFightersFromCapitalShip()` in `animate()`) - Enemy fighters scramble and launch immediately (3 fighters per capital dreadnought) the instant each carrier emerges from hyperspace, followed by continuous periodic 8-13s reinforcement launches from the ventral bays.
*   loadDominionFighterModel() / createEnemyInterceptorMesh() - Loads the solid binary glTF Dominion Fighter model (`fbx/dominion_fighter.glb`, 250k faces, 6.3MB, 100% watertight boundary-preserved geometry) via `THREE.GLTFLoader`, centered and scaled to ~12 units, with 4 rear crimson plasma thrusters.
