...
*   createTitanExcavationAndGoldenGate() - Spawns the superheated molten excavation crater, fractured ice crust rim, boiling liquid methane vapor geyser particle pool, expanding planetary shockwaves, and ionization lightning on Titan's Kraken Mare. Also spawns the colossal Ancient Golden Gate ring with 12 anchor obelisks, luminous violet focus crystals, rotating runic glyph bands, and a dense 2D planar circular cosmic particle wormhole with dual counter-rotating swirls (clockwise violet/cyan/magenta swirl and counter-clockwise gold/cyan/white swirl flowing in opposite directions in the 2D aperture) and peripheral boundary energy corona. Gate is not visible initially until ice melts.
...
*   createCapitalShip() / spawnDominionCapitalFleet() / buildSingleCapitalShip() / DOMINION_FLEET_CONFIG - Loads and templates the optimized textured binary glTF Dominion Capital Ship model (`fbx/dominion_capital_ship.glb`, 236k faces, 14.9MB, KDTree UV-projected 3-channel PBR textures) and dynamically instantiates the 5-ship Dominion Siege Fleet (*Shadow of Aythelgard*, *Titan's Bane*, *Iron Sovereign*, *Void Reaver*, *Blood Eclipse*) upon Mission 3 hyperspace emergence in a locked military battle line facing towards the player and The Crest (`fleetFacing = (0, 0, 1)`) along the original emergence flight corridor (`flightDirection = (0, 0, -1)`), with rear engines in the back (`z: 535`/`z: 525`), and recessed front spinal weapon cavities (`z: -475`).
*   initCapitalShipSpinalBeams() / updateTitanExcavationAndTractorBeams() - Multi-layer spinal beam arrays on all 5 capital dreadnoughts driving a 4-phase cinematic sequence:
    1. Phase 0: Radiant Gold Bombardment Beams firing strictly from the front prow of each capital ship directly to the gate site on Titan, melting the methane ice crust while the gate is completely hidden beneath the ice.
    2. Phase 1: Ice melts -> gold bombardment beams cease -> huge golden ancient gate is exposed and free on the surface.
    3. Phase 2: Convergent Purple Graviton Tractor Beams engage and slowly haul the golden gate off Titan into space, turning it on its side (vertical aperture) and expanding it to 2x scale, while Dominion secondary strikes hit *The Crest*, triggering a massive 2-second fireball explosion that dissipates while shattered debris scatters outward into space.
    4. Phase 3: Golden Gate arrives in space (2x size, standing on its side) -> tractor beams cease -> dense particle slipspace event horizon vortex ignites to full intensity. Exactly 15 seconds after The Crest is destroyed, the full voice-acted Titan Gate Wormhole Cinematic automatically begins (with full player invulnerability), concluding with an overdrive flight straight through the center of the ring into the event horizon vortex.
*   updatePlanetaryExcavationAndTractorAudio() / playTheCrestExplosionAudio() - Real-time procedural Web Audio API sound synthesis for sub-bass planetary graviton tractor drone, dark energy beam resonance, boiling vapor hiss, ethereal ancient chimes, hull fracture cracks, 24 Hz infrasound reactor core boom, and roaring plasma fireballs.
...
*   loadSkullRaiderModel() / createSkullRaiderTexture() / createPirateShipMesh() - Loads the Skull Raider binary glTF model with procedural dark weathered gunmetal hull textures (carbon plating, industrial panel seams, rivets, rust weathering, crimson/orange war stripes, and pirate skull insignia) with PBR bump/roughness maps and dual orange thrusters for pirate asteroid patrols.
...
*   createIrregularAsteroidGeometry() / createAsteroidRockTexture() - Procedurally generates solid, realistic rocky asteroid meshes with low dynamic range harmonic surface undulations (clamped within ±12% displacement) and dark silicate cratered canvas textures.
...
*   approachTitanBombardment() / toggleTractorExtraction() / approachAncientGate() - Interactive HUD camera and positioning controls to observe the Titan dark-energy crater excavation, tractor ring ascent, and the stabilized Golden Ancient Gate.
*   startTitanGateCinematic() / playNextCinematicLine() / finishTitanGateCinematic() / stopTitanGateCinematic() - Multi-stage cinematic cutscene engine driving the Act I climax. Features synchronized ElevenLabs voice acting for Elias Vance and Kaylen Vance, military comms transmission card HUD overlay with animated audio waveforms, dynamic multi-angle director camera cuts, autonomous full-overdrive interceptor flight into the Ancient Wormhole vortex, and seamless blackout fade to black upon crossing the threshold.
*   triggerMissionDebriefSequence() / toggleTacticalArmoryDrawer() / scrambleNextMission() - Wing Commander 3 inspired seamless flight deck transition system. Combines continuous 3D WebGL background flight with post-mission performance metrics (kills, accuracy, hull integrity, wingman status), interactive character radio chatter, non-blocking slide-out Tactical Armory drawer, and instant scramble into the next mission vector.
...
*   `data/voices.json` & `docs/voice_casting.md` - Persistent character voice casting registry for ElevenLabs API, storing voice IDs, stability, and similarity parameters across characters (Kaylen Vance, Elias Vance, Ancient AI / Althea, Story Narrator, Jax, Kayl, Lyra, Regent Vaylen).
