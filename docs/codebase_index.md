...
*   loadVoidInterceptorModel() / buildDetailedShipMesh() - Loads the ultra-fast binary glTF model (`fbx/void_interceptor.glb`, optimized to 45k triangles, 1.6MB) with correct forward orientation along -Z (`model.rotation.set(-Math.PI / 2, -Math.PI / 2, 0)`), twin engine exhaust glow assemblies (cavities, thruster discs, core discs, halos).
