...
# Match the end of the spaceTitan block precisely
pattern = r"(\s*if\s*\(statusTag\.innerText\s*===\s*'TITAN REPULSE'\)\s*\{\s*statusTag\.innerText\s*=\s*'STABLE';\s*statusTag\.style\.color\s*=\s*'#f59e0b';\s*\}\s*\}, 500\);\s*\}\s*\}\s*\}\s*\})"
content = re.sub(pattern, r"\1" + "\n" + collision_code, content)
